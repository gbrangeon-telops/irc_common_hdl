...
module rs232_syscon (
   ...
   parameter ADR_DIGITS_PP = 3;             // # of hex digits for address.
   ...
   // Output registers (PDU)
   reg [`NIBBLE_SIZE*ADR_DIGITS_PP-1:0] adr_o;
   reg we_o  ;
   reg stb_o ;
   reg cyc_o ;
   reg [`NIBBLE_SIZE*DAT_DIGITS_PP-1:0] dat_o;
   
   // Input registers (PDU)
   reg [`NIBBLE_SIZE*DAT_DIGITS_PP-1:0] dat_i_reg;
   reg ack_i_reg;
         ...
         case (m1_state) // synthesis parallel_case
            ...
            m1_execute :
               begin
                  ...
                  else if (ack_i_reg && (command == `CMD_R)) begin
                        m1_next_state <= m1_rd_send_dat_sr; // Leads to a new data field.
                        reset_rd_digit_count <= 1;
                        reset_msg_offset <= 1;
                        incr_adr_offset <= 1;             // move to the next address
                     end
                  else if (ack_i_reg) begin
   ...
   always @(posedge clk_i)
      begin
         ...
         else if (capture_dat) dat_sr <= dat_i_reg;
