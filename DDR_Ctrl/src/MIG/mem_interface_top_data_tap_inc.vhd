...
architecture arch of mem_interface_top_data_tap_inc is
  ...
  signal chan_sel_int     : std_logic_vector(DatabitsPerStrobe-1 downto 0) := add_const6b((DatabitsPerStrobe-1) downto 0);
...
begin
  ...
  BIT_BOUNDARY_CHECK <= '1' when ((calibration_dq_r = add_const3(DatabitsPerStrobe-1 downto 0))
                                  or (calibration_dq_r = add_const5b(DatabitsPerStrobe-1 downto 0)))
                        else '0';
  COMPARE_BIT <= '1' when (calibration_dq_r = add_const5b(DatabitsPerStrobe-1 downto 0))
                 else '0';
  ...
  begin
    if (CLK'event and CLK = '1') then
      if (RESET_r1 = '1') then
        chan_sel_int <= add_const6b((DatabitsPerStrobe-1) downto 0);
      elsif(CTRL_CALIB_START = '1') then
        if (CHAN_DONE = '1') then
